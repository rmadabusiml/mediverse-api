...
APP_DIR=app
APP_NAME=health-universe-api
fn_bucket=dml-dev-deployment
...
pip3 install -r $REQS_TXT_PATH --no-cache-dir --target=$DEPS_DIR
...
ls -ltr $FN_ZIP_FILE 
aws s3 cp $FN_ZIP_FILE s3://$fn_bucket/raghu/$APP_NAME/
